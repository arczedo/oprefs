...
import Foundation
func run(_ path: String, callback: ((String) -> Void)?) {
    let pipe = Pipe()
//    let path = "/Users/jpz3562/Z/W/prefs/macOS/zsh/f12/\(cmd)"
    do {
        ...
        task.execute(withArguments: nil) { err in
            callback?(String(data: pipe.fileHandleForReading.readDataToEndOfFile(), encoding: .utf8) ?? "")
        }
    } catch(let e) {
...
//        process.environment = ["ZDOTDIR": "/Users/jpz3562/.zshrc"]
        let pipe = Pipe()
